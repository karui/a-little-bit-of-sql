...
-- fill in resume_specialization
INSERT INTO resume_specialization (
	resume_id,
	specialization_id
)
SELECT
      (random()*99999+1)::int AS resume_id,
      (random()*99+1)::int AS spezialization_id
FROM generate_series(1, 50000) AS g(i);


-- fill in vacancy_body_specialization
-- truncate table vacancy_body_specialization; 
INSERT INTO vacancy_body_specialization (
	vacancy_body_id,
	specialization_id
)
SELECT
      (random()*9999+1)::int AS vacancy_body_id,
      (random()*99+1)::int AS spezialization_id
